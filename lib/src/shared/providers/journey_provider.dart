...
const List<String> legacyMoodEmojis = <String>['😢', '☹️', '😐', '🙂', '😄'];

int moodIndexFromEmoji(String moodEmoji) {
  final index = legacyMoodEmojis.indexOf(moodEmoji);
...
class JournalEntry extends HiveObject {
  ...
  String get resolvedMoodEmoji {
    if (moodEmoji.isNotEmpty) {
      return moodEmoji;
    }
    final index = moodIndex.clamp(0, legacyMoodEmojis.length - 1);
    return legacyMoodEmojis[index];
  }
}
...
class DraftJournalNotifier extends StateNotifier<DraftJournalEntry> {
  ...
  void setMood(String emoji) {
    state = state.copyWith(moodEmoji: emoji, hasChanges: true);
  }
  ...
  void submitEoD({required int totalRoutines, required int totalSupplements}) {
    ...
    final entry = JournalEntry(
      ...
      date: today,
      eodDate: today,
      moodIndex: moodIndexFromEmoji(state.moodEmoji),
  ...
  void loadExistingEntry(JournalEntry entry) {
    state = DraftJournalEntry(
      moodEmoji: entry.resolvedMoodEmoji,
