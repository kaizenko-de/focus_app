...
class _HistoryCardState extends State<_HistoryCard> {
  ...
  Widget build(BuildContext context) {
    final topWinObj = widget.entry.wins.isNotEmpty
        ? widget.entry.wins.firstWhere(
            (w) => w.isHighlighted,
            orElse: () => widget.entry.wins.first,
          )
        : null;
    
    final topWin = topWinObj?.text ?? 'No wins recorded';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_expanded) ...[
            ...widget.entry.wins.map((win) {
              // Skip the win that's already shown as TOP WIN
              if (win.id == topWinObj?.id) return const SizedBox.shrink();
                  ...
                  children: [
                    ...
                    if (win.isHighlighted)
                      Container(
                        ...
                        height: 24,
                        decoration: BoxDecoration(
                          color: AppColors.goldenSelected.withValues(alpha: 0.22),
